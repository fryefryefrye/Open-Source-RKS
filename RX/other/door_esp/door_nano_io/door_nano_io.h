...
void requestEvent() 
{
	//for (char i = 0; i < IIC_SLAVE_RECV_BYTE-1; i++)
	//{
	//	IicSlaveSend[IIC_SLAVE_SEND_BYTE-1] = IicSlaveSend[IIC_SLAVE_SEND_BYTE-1] + IicSlaveRecv[i];
	//}
...
void OnTenthSecond()
{
	...
	if ((IicSlaveRecv[0] == (unsigned char)(~IicSlaveRecv[1]))&&(IicRecvTimeOutTenthSecond<2))
	{
		digitalWrite(RELAY_UNLOCK	,!((IicSlaveRecv[0])&1));
		digitalWrite(RELAY_LOCK		,!((IicSlaveRecv[0]>>1)&1));
		//digitalWrite(RELAY_OPEN		,!((IicSlaveRecv[0]>>2)&1));
		//digitalWrite(RELAY_CLOSE	,!((IicSlaveRecv[0]>>3)&1));
		digitalWrite(BUZZ			,!((IicSlaveRecv[0]>>4)&1));
	}
	else
	{
	...
	//printf("isKeyPressed %d %d %d %d out:%d in:%d closed:%d 0x%02X\r\n"
	//	,isKeyPressed[0]
	//,isKeyPressed[1]
	//,isKeyPressed[2]
	//,isKeyPressed[3]
...
bool KeyState(unsigned char KeyIndex)
{
	...
	case 0:
		...
		return (analogRead(KEY_OUT1)>900);
		break;
	case 1:
		//return digitalRead(KEY_OUT2);
		return (analogRead(KEY_OUT2)>900);
		break;
	case 2:
