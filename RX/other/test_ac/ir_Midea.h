...
const uint8_t kMideaACFanAuto = 5;  // 0b000
const uint8_t kMideaACFanLow = 4;   // 0b001
const uint8_t kMideaACFanMed = 2;   // 0b010
const uint8_t kMideaACFanHigh = 1;  // 0b011
...
class IRMideaAC {
 public:
  ...
  uint8_t getMode();
  void setRaw(unsigned char *newState);
  unsigned char *  getRaw();
  void calc();
  bool checksum();
unsigned char CodetoTemperature(unsigned char code);
  unsigned char TemperatureToCode(unsigned char Temp);




 private:
  unsigned char remote_state[6];
  unsigned char Temp;
  unsigned char Mode;
