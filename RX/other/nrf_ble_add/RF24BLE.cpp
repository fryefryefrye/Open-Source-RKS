...
{
	WritingPipeAddress = reverseBits(0xD6);
	WritingPipeAddress <<= 8;
	WritingPipeAddress |= reverseBits(0xBE);
	WritingPipeAddress <<= 8;
	WritingPipeAddress |= reverseBits(0x89);
	WritingPipeAddress <<= 8;
	WritingPipeAddress |= reverseBits(0x8E);
}
...
void RF24BLE::recvBegin(uint8_t payloadSize, uint8_t channel, uint64_t pipeAddress){
	...
	begin();
	_radio.setChannel(RF24BLE::chRf[_recvChannel]);
	_radio.setPayloadSize(payloadSize);
	_radio.openReadingPipe(1, 0xe76b7d9171);
	_radio.startListening();
}

void RF24BLE::recvRestar(uint8_t channel)
{
	_recvChannel = channel;
	//_radio.stopListening();
	//_radio.closeReadingPipe(1);
	_radio.disableCRC();
	//_radio.powerUp();
	//_radio.setAutoAck(false);
	_radio.setAddressWidth(4);
...
void RF24BLE::setRequestMAC(uint8_t * Smac,uint8_t * Dmac)
{
	//length of payload is entered in byte no.1
	_length = 2;
	_packet[_length++] = Smac[0];
	_packet[_length++] = Smac[1];
	_packet[_length++] = Smac[2];
	_packet[_length++] = Smac[3];
	_packet[_length++] = Smac[4];
	_packet[_length++] = Smac[5];

	_packet[_length++] = Dmac[0];
	_packet[_length++] = Dmac[1];
	_packet[_length++] = Dmac[2];
	_packet[_length++] = Dmac[3];
	_packet[_length++] = Dmac[4];
	_packet[_length++] = Dmac[5];
...
// 		printf("0x%02X,",(unsigned char)_packet[i]);
// 	}
// 	printf("  Len = %d ch = %d \r\n",_length,channel);
	...
	//_radio.write(_packet, _length,false);
#if DEBUG == 1
...
void RF24BLE::printPacket(){
	...
	printf("  Len = %d",_length);
}
uint8_t RF24BLE::getPacketLengthCurr(){ return _length; }

uint8_t RF24BLE::recvPacket(uint8_t *input, uint8_t length){
	...
	bleWhiten(input, length, bleWhitenStart(RF24BLE::chLe[_recvChannel]));
