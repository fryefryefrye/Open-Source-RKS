...
public:
	...
	void recvChannelRoll();
	void recvRestar(uint8_t channel);
	void setMAC(uint8_t m0, uint8_t m1, uint8_t m2, uint8_t m3, uint8_t m4, uint8_t m5);
	void setRequestMAC(uint8_t * Smac,uint8_t * Dmac);
	...
	*/
	...
	uint8_t recvPacket(uint8_t *input, uint8_t length);
	uint8_t _recvChannel = 0; 
	unsigned long WritingPipeAddress;
};
