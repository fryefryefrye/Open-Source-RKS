...
void loop() 
{
	...
	if (UdpAvailable == sizeof(tDcMeterCommand))
	{
		//MyPrintf("m_WiFiUDP.available() = %d\r\n",UdpAvailable);
		//printf("m_WiFiUDP.available() = %d\r\n",UdpAvailable);
		tDcMeterCommand tempDcMeterCommand;
		m_WiFiUDP.read((char *)&tempDcMeterCommand,sizeof(tDcMeterCommand));
		LastServerUpdate = 0;
	}
}
...
void OnSecond()
{
	...
	{
		DcMeterData.volt = BusVoltage * 1000;
	}

	if ((Current >= 0)&&(Current<(float)5000))
	{
		DcMeterData.Current = Current;
	}



	printf("Current:%.2f BusVoltage:%.2f ShuntVoltage:%.2f\r\n",Current,BusVoltage,ShuntVoltage);

	m_WiFiUDP.beginPacket("192.168.0.17", 5050);
	m_WiFiUDP.write((const char*)&DcMeterData, sizeof(tDcMeterData));
	m_WiFiUDP.endPacket(); 
}
