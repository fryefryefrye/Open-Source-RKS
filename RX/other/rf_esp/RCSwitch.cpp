...
static const RCSwitch::Protocol PROGMEM proto[] = {
  ...
  { 350, {  2, 25 },{  16, 2 }, {  1,  2 }, {  2,  1 }, false },    // protocol 2
  //{ 350, {  1, 7 },{  26, 4 }, {  1,  2 }, {  2,  1 }, true },    // protocol 2
...
void RCSwitch::send(unsigned char * code, unsigned int length) 
{
...
void RCSwitch::send_duya(unsigned char * code, unsigned int length) 
{
	unsigned char ByteIndex;
	if (this->nTransmitterPin == -1)
		return;
	...
	for (int nRepeat = 0; nRepeat < nRepeatTransmit; nRepeat++) 
	{
		...
		this->transmit(protocol.syncFactor);
		this->transmit(protocol.syncFactor2);

		for (int i = 0; i < length; i++) 
		{
			ByteIndex = (i/8);
			
			if (code[ByteIndex] & (1L << (7-i%8)))
			{
				this->transmit(protocol.one);
			}
			else
...
void RCSwitch::transmit(HighLow pulses) {
  ...
  {
	  yield();
	  if (micros() - StartMicros > this->protocol.pulseLength * pulses.high)
	  {
		  break;
	  }
  }
  ...
  {
	  yield();
	  if (micros() - StartMicros > this->protocol.pulseLength * pulses.low)
	  {
		  break;
	  }
  }
