...
unsigned char IicNanoSend[IIC_NANO_SEND_BYTE];
unsigned char IicNanoRecv[IIC_NANO_RECV_BYTE];
...
void IicExchange()
{
	//IicNaSend[1] = (unsigned char)(~IicEspSend[0]);
	Wire.beginTransmission(8); /* begin with device address 8 */
	//Wire.write("Hello Arduino");  /* sends hello string */
	Wire.write(IicNanoSend,IIC_NANO_SEND_BYTE);

	Wire.endTransmission();    /* stop transmitting */

	Wire.requestFrom(8, IIC_NANO_RECV_BYTE); /* request & read data of size 13 from slave */
	unsigned char RecvIndex = 0;
	while(Wire.available()){
		IicNanoRecv[RecvIndex] = Wire.read();
		RecvIndex++;
	...
	IicNanoSend[IIC_NANO_SEND_BYTE-1]++;
...
void OnTenthSecond()
{
	...
	//{
		...
		//if (IicRecvTimeOutTenthSecond>1)
		//{
		//	printf("IIC restart\r\n");
		//	Wire.end();
		//	Wire.begin(8);
		...
		for (char i = 0; i < IIC_NANO_RECV_BYTE; i++)
		{
			printf("0x%02X ",IicNanoRecv[i]);
		}
		printf("\r\n");
	...
	if (IicNanoRecv[0] == (unsigned char)(~IicNanoRecv[1]))
	{
		digitalWrite(RELAY_LOCK		,((IicNanoRecv[0])&1));
		digitalWrite(RELAY_UNLOCK	,((IicNanoRecv[0]>>2)&1));
		digitalWrite(RELAY_OPEN		,((IicNanoRecv[0]>>3)&1));
		digitalWrite(RELAY_CLOSE	,((IicNanoRecv[0]>>4)&1));
		digitalWrite(BUZZ			,((IicNanoRecv[0]>>5)&1));
	}
	else
	{
	...
	IicNanoSend[0] = TagState;
	IicNanoSend[1] = (unsigned char)(~TagState);
	...
	IicNanoSend[2] = TempState;
	IicNanoSend[3] = (unsigned char)(~TempState);
}
